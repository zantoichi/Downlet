...
- Preserve user control: manual resizing, maximizing, snapping, or full-screen placement takes precedence over automatic sizing. App-managed sizing may grow a user-sized window only when required to keep essential content usable and must not forcibly shrink it.
...
- Update the design-review harness, automated checks, and exact-commit evidence to cover compact launch, growth, shrink, interruption, manual override, placement, scaling, themes, and all six states.
...
- `state-driven-window-sizing`: State-to-tier mapping, content-fit bounds, coordinated window motion, manual-resize ownership, screen-bound handling, reduced-motion behavior, and observable review evidence.
...
- Dependencies: no new runtime or animation dependency. Reuse Compose Desktop/Jewel window state, Compose animation primitives, Kotlin coroutines already present, and AWT window/screen APIs only where platform bounds require them.
