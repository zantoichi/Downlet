...
- JBR `25` development/application runtime
...
IntelliJ Platform sources but no published standalone coordinate in the current release table. Compose Hot Reload 1.2.0
runs on JBR 25 but requires projects to target Java 21 or earlier, so compile target and runtime are explicit and
separate.
...
| Plain themed title bar                | Jewel `TitleBar` inside `DecoratedWindow`                                                                   | One centered/left `Text("Downlet")` only                                          | High                | Do not add project stripe, menu, toolbar, breadcrumbs, or custom hit regions. Compose capture omits chrome; use Computer Use full-window proof. |
...
| Compile/runtime compatibility         | Kotlin `JvmTarget.JVM_21`, Java `--release 21`, JBR 25 launcher/runtime                                     | Gradle configuration only                                                         | High                | IntelliJ dependency/build inspection must confirm target 21 while the running process reports JBR 25.                                           |
| Normal startup theme                  | Compose Foundation `isSystemInDarkTheme()`                                                                  | Light fallback plus explicit review override                                      | High                | Read at normal launch; no live in-session Windows-theme watching. Restart must pick up a changed preference.                                    |
...
| Rendered client-area evidence         | Compose Hot Reload MCP `status`, `list_windows`, screenshots, semantics, interactions, resize, errors, logs | Project `hotMcpServer` configuration                                              | High after scaffold | G1 cannot pass until the MCP connects to the exact running commit; screenshots do not prove title chrome.                                       |
| Native window-frame evidence          | Codex Computer Use `Windows.Graphics.Capture` plus manual Windows interaction record                        | Evidence capture only                                                             | High                | Tie light/dark captures to same commit/run; verify title-bar theme, controls, drag, and maximize/restore.                                       |
...
- Official Hot Reload prerequisites: project target Java 21 or earlier; version 1.2.0 uses JBR 25 as its default runtime
...
- target-21 compilation and JBR-25 runtime agreement;
- decorated title-bar behavior under JBR 25, proven outside Compose client-area screenshots;
- startup Windows theme detection and light fallback;
