...
### Unicode download correction and installed Node support

The Greek-titled video `Ivw9X-nYLwQ` exposed a Windows stdin encoding mismatch: yt-dlp reads cached JSON with the Windows locale even when `--encoding UTF-8` controls its output. Raw Unicode was corrupted, and the affected title failed in the byte-limited output template with a surrogate encoding error. Downlet now escapes non-ASCII UTF-16 code units as JSON `\uXXXX` sequences at the shared stdin boundary. Decoding preserves the original text, including emoji; cache sanitization and the prepared-process consent boundary are unchanged. The real CLI regression failed on a corrupted filename before the fix and passes for both prepared and direct downloads afterward.

Normal requests now enable installed Node alongside the verified bundled QuickJS-NG runtime. yt-dlp handles version compatibility, prioritizes Node, and tries QuickJS for unresolved challenges if Node is unavailable or fails. A nonblank `DOWNLET_QUICKJS` override keeps QuickJS-only selection. No Node installation, runtime download, custom compatibility probe, or application-level extraction retry was added. The pinned yt-dlp already supplies this behavior through its [provider director](https://github.com/yt-dlp/yt-dlp/blob/2026.08.19/yt_dlp/extractor/youtube/jsc/_director.py).

Do not enable `youtube-ejs:jitless=true` for Node with pinned yt-dlp `2026.08.19`: its Node provider passes `--v8-flags=--jitless`, which Node 24.17.0 rejects. This was used only to exercise provider fallback during diagnosis, not added to application arguments. Normal Node execution retains yt-dlp's `--permission` restriction.

Measured after packaging, with no concurrent build, using the same video, Firefox session, yt-dlp 2026.08.19, QuickJS-NG 0.16.2, and Node 24.17.0. Three alternating runs per configuration measured the complete CLI metadata lookup, including browser-cookie extraction and network requests:

| Runtime configuration | Runs (seconds) | Median |
| --- | --- | --- |
| QuickJS only | 11.029, 11.523, 13.617 | 11.523 s |
| Node with QuickJS fallback | 5.140, 5.478, 5.595 | 5.478 s |

The median elapsed time fell by 52.5%. Every run returned the same 35 format IDs (including storyboard entries) and selected audio format 251. Separate checks for a missing Node path, an incompatible Node 20 version stub, and a deliberately failed Node provider all resolved successfully through QuickJS with those same formats. These are local CLI timings, not UI latency guarantees. The diagnostic script and sanitized results are under ignored `build/runtime-verification/`; no cookies or signed media URLs are saved in the results.

Validation: `check` passed with 103 tests, `smokeTest` passed with 11 scenarios, and portable packaging/native launcher checks passed. The rebuilt portable is 86,414,848 bytes (82.41 MiB; 198.30 MiB extracted). Computer Use was stopped with Escape during the rebuilt-app launch attempt, so a full download through that executable and FFprobe verification of its output remain unverified. The successful automated Unicode regression uses the real yt-dlp executable and a local HTTP media fixture.

### Thumbnail, theme, and audio-size follow-up

A delayed-preview regression reproduced the missing thumbnail: the state holder discarded the entire optional preview after three seconds. Preview now keeps the existing twenty-second HTTP timeout per request, independently of resolution. Download no longer cancels the image request, and a late thumbnail survives progress and completion without changing resolved identity or selections. Editing the URL or cancelling still cancels obsolete work.

The reported Riftbound video's public metadata and JPEG both returned HTTP 200 with the application's Java HTTP client settings: 1.030 s and 0.361 s using IPv4-only networking. The regression deliberately delays the preview beyond four seconds; another covers arrival during transfer and subsequent completion. These network samples establish reachability, not that every response is equally fast.

Preview XML and format JSON parsing/cache population now run on the IO dispatcher instead of resuming on the Compose UI thread. The theme icon reads animation state in its Canvas draw phase, avoiding animation-frame recomposition, and its configured morph duration drops from 160 ms to 100 ms. The smoke test checks interrupted and disabled icon motion using rendered pixels. This follows Compose's [deferred state-read guidance](https://developer.android.com/develop/ui/compose/performance/bestpractices#defer-reads). No claim is made that all click-to-paint latency is bounded by the icon duration.

Original audio shows exact reported size first, reported approximate size second, then a duration/bitrate estimate if possible. Each MP3 preset shows estimated output size using its target bitrate (190 kbps for high-quality VBR). Estimates carry a tilde, unavailable sizes are omitted, and the displayed units reuse the existing video formatter. The MP3 estimate describes the resulting file, not the source bytes transferred.

A full original-audio diagnostic transfer for `Huo0Sy_OAjw` completed in 66.325 s using the authorized Firefox session. Its 193 native CLI progress events included a maximum 1.588 s gap with no retry messages. Thus brief pauses reproduce outside Downlet; no artificial progress smoothing or extra retry layer was added. FFprobe verified one Opus audio stream, duration 7994.821 s, and 123,861,205 bytes. The diagnostic used PATH FFprobe 8.0.1. A concurrent check run overlapped part of the transfer, so the total time is not an isolated throughput benchmark. Sanitized measurements are in ignored `build/runtime-verification/riftbound-transfer.json`.

Follow-up validation: `check` passed (105 tests, none skipped), `smokeTest` passed (11 scenarios), and `packageWindowsSingleExe` passed including native launcher checks. The rebuilt portable is 82.41 MiB (198.18 MiB extracted). A focused launch of that executable displayed the actual Riftbound thumbnail, retained it across theme changes, showed Original size 118.1 MB and MP3 160 kbps estimated output 152.5 MB, and completed Original download to `D:\Downloads`. FFprobe confirmed its published file contains Opus audio, duration 7994.821 s, size 123,861,205 bytes. Theme latency was checked functionally, not measured with a click-to-paint instrument.
...
- **Progressive resolution:** optional preview and authoritative resolution overlap. Preview retains bounded per-request network timeouts; Ready appears as soon as formats arrive. Late thumbnails preserve selected quality, mode, consent, and destination. Resolving has a 30-second deadline with process-tree cancellation; transfers and conversion do not inherit that deadline.
